...
git commit --allow-empty -m "login button alignment"
...
git commit --allow-empty -m "typo in release"
...
Write-Host "Creating hotfix/login-bug from main"
git checkout main
git checkout -b hotfix/login-bug
git push -u origin hotfix/login-bug --force
Start-Sleep -Seconds 5

git commit --allow-empty -m "critical security issue"
git push origin hotfix/login-bug --force
Start-Sleep -Seconds 5

Write-Host "Merging hotfix into main and tagging"
git checkout main
git merge hotfix/login-bug --no-ff -m "Merge branch 'hotfix/login-bug' into main"
git tag "1.1.1" -f
git push origin main --force
git push origin --tags --force
Start-Sleep -Seconds 5
